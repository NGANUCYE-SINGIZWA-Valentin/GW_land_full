...
export const SellerInquiries: React.FC = () => {
    ...
    const QUICK_ACTION_CHIPS = [
        { label: 'Schedule Site Visit', icon: <Calendar size={13} />, text: 'Hello! This property is available for a site visit. When are you free to inspect it?' },
        { label: 'Send UPI Documents',  icon: <FileCheck size={13} />, text: 'Hi! All land title and UPI documentation are verified and ready for review.' },
        { label: 'Discuss Price Offer', icon: <DollarSign size={13} />, text: 'Thank you for your interest! What offer price do you have in mind?' },
    ];
    ...
    const columns: ColumnConfig<Conversation>[] = [
                ...
                <div className="flex items-center gap-3">
                    <div className="relative">
                        ...
                        {Number(c.unread_count || 0) > 0 && (
                            <span className="absolute bottom-0 right-0 w-3 h-3 rounded-full bg-amber-500 ring-2 ring-white animate-pulse" />
                        )}
                    </div>
                    <div className="min-w-0">
                        <h4 className="text-sm font-bold text-slate-800 truncate">{otherPartyName(c)}</h4>
                        <p className="text-[11px] text-slate-400 font-medium">Buyer Inquiry</p>
                    </div>
                </div>
            ),
        },
        {
            ...
            render: (c) => (
                <div className="flex items-center gap-2 max-w-[200px]">
                    <div className="p-1.5 rounded-lg flex-shrink-0" style={{ background: 'rgba(84,181,187,0.12)', color: '#54B5BB' }}>
                        <Building2 size={14} />
                    </div>
        ...
        {
            header: 'Last Message',
            render: (c) => <span className="text-slate-500 text-xs truncate max-w-[240px] block font-medium">{c.body}</span>,
        },
        {
            header: 'Date',
            render: (c) => formatRelativeTime(c.created_at),
            cellClassName: 'text-slate-400 text-xs font-semibold',
        },
        {
            header: 'Status',
            render: (c) => Number(c.unread_count || 0) > 0 ? (
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-extrabold bg-amber-50 text-amber-600 border border-amber-200">
                    <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-ping" /> {c.unread_count} new
                </span>
            ) : (
        ...
        },
        {
            ...
            render: (c) => (
                <button
                    ...
                    className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold rounded-xl transition-all hover:scale-105 border"
    ...
    return (
        <div className="space-y-8 font-sans antialiased">
            ...
            <div className="relative overflow-hidden rounded-3xl p-8 text-white shadow-2xl border border-slate-800"
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    <div>
                        ...
                        <h1 className="text-2xl md:text-3xl font-black tracking-tight text-white">Buyer Inquiries & Leads</h1>
                        <p className="text-slate-300 text-sm mt-1 max-w-xl">Respond to site visit requests, negotiate property transactions, and manage buyer leads.</p>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="px-4 py-2.5 rounded-2xl bg-white/10 backdrop-blur-md border border-white/10 text-center">
                            <p className="text-[11px] font-bold text-slate-300 uppercase tracking-wider">Total Leads</p>
                            <p className="text-xl font-black text-white">{kpiCounts.all}</p>
                        </div>
                        <div className="px-4 py-2.5 rounded-2xl backdrop-blur-md border text-center"
                            style={{ background: 'rgba(245,158,11,0.2)', borderColor: 'rgba(245,158,11,0.3)' }}>
                            <p className="text-[11px] font-bold text-amber-300 uppercase tracking-wider">Unread</p>
                            <p className="text-xl font-black text-amber-400">{kpiCounts.unread}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 max-w-xl">
                <StatCard title="Total Inquiries" value={String(kpiCounts.all)} icon={<MessageSquareText size={20} />}
                    accentGradient="teal" showSubtext={false} isActive={activeFilter === 'all'} onClick={() => setActiveFilter('all')} />
                <StatCard title="Unread Messages" value={String(kpiCounts.unread)} icon={<MailQuestion size={20} />}
                    accentGradient="amber" showSubtext={false} isActive={activeFilter === 'unread'} onClick={() => setActiveFilter('unread')} />
            </div>

            {/* Table */}
            <TableBlueprint data={filtered} columns={columns} isLoading={loading}
                emptyMessage={loading ? 'Loading conversations...' : 'No buyer inquiries found.'}
                searchPlaceholder="Search buyer or property..." searchKeys={['sender_name', 'receiver_name', 'listing_title']}
                totalItems={filtered.length} hasPrevPage={false} hasNextPage={false} />

            {/* ── Centered Chat Modal (replaces side drawer) ── */}
            <ChatModal
                isOpen={isModalOpen}
                onClose={() => setIsModalOpen(false)}
                title={selectedConversation ? otherPartyName(selectedConversation) : 'Conversation'}
                subtitle="Live Inquiry Thread"
                icon={<MessageSquareText size={18} />}
                footer={selectedConversation && (
                    <div className="space-y-3">
                        {/* Quick chips */}
                        <div>
                            <p className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 mb-2">Quick Responses</p>
                            <div className="flex flex-wrap gap-2">
                                {QUICK_ACTION_CHIPS.map((chip, idx) => (
                                    <button key={idx} onClick={() => handleReply(chip.text)} disabled={sending}
                                        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-xl border transition-all hover:scale-105 active:scale-95 disabled:opacity-50"
                                        style={{ color: '#1B395F', background: 'rgba(84,181,187,0.1)', borderColor: 'rgba(84,181,187,0.25)' }}>
                        ...
                        <div className="flex gap-2 w-full">
                            <input type="text" value={replyBody} onChange={(e) => setReplyBody(e.target.value)}
                                ...
                                className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:border-transparent transition-all"
                                style={{ '--tw-ring-color': 'rgba(84,181,187,0.4)' } as React.CSSProperties}
                            />
                            <button onClick={() => handleReply()} disabled={sending || !replyBody.trim()}
                                className="px-5 py-3 text-white font-bold rounded-2xl shadow-lg flex items-center gap-2 text-sm disabled:opacity-40 transition-all hover:scale-105 active:scale-95 flex-shrink-0"
                ...
                {selectedConversation && (
                    <div className="space-y-5">
                        ...
                        <div className="p-4 rounded-2xl border flex items-center gap-3"
                            style={{ background: 'rgba(84,181,187,0.08)', borderColor: 'rgba(84,181,187,0.2)' }}>
                                ...
                                return (
                                    <div key={m.id} className={`flex flex-col ${isMine ? 'items-end' : 'items-start'}`}>
                                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
                                            {isMine ? 'You (Seller)' : otherPartyName(selectedConversation)}
                                        </span>
