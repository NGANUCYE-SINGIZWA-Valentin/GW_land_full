...
export const RegisterPage: React.FC = () => {
  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!role || (role !== 'Buyer' && role !== 'Seller')) {
      setRoleError(t('auth.selectRoleToContinue'));
      return;
    }
  ...
  const handleSocialClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    if (!role || (role !== 'Buyer' && role !== 'Seller')) {
      e.preventDefault();
      setRoleError(t('auth.selectRoleBeforeContinuing'));
    }
  };
        ...
        <div className="px-1 pt-0.5">
          <label className="flex items-start gap-2 cursor-pointer group">
            ...
            <span className="text-xs font-semibold text-slate-600 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white transition-colors leading-relaxed">
              {t('auth.agreeToThe')}{' '}
              <Link to="/terms-conditions" className="text-brand-primary dark:text-brand-secondary font-extrabold hover:underline">{t('auth.termsOfService')}</Link>
              {' '}{t('auth.and')}{' '}
              <Link to="/privacy-policy" className="text-brand-primary dark:text-brand-secondary font-extrabold hover:underline">{t('nav.privacy')}</Link>
            </span>
          </label>
        </div>
